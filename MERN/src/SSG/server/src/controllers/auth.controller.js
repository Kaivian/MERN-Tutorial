...
class AuthController {
   ...
   * Creates a new user and automatically logs them in (Auto-Login pattern).
   */
  async register(req, res) {
    try {
      const { username, email, password } = req.body;

      // 1. Basic validation (Service should handle deep validation like regex)
      if (!username || !email || !password) {
        return res.error('Missing required fields: username, email, or password', 400);
      }

      // 2. Call Service to create user
      // Assumes service returns tokens for auto-login logic
      ...
      // 3. Set Cookies immediately (Auto-Login)
      this.setAuthCookies(res, { accessToken, refreshToken });

      // 4. Success Response (201 Created)
      logger.success(`[Auth] New user registered: ${user.username}`);
      res.success({ user }, 'Registration successful', 201);

    } catch (error) {
      // Check for duplicate key errors (common in registration)
      const isConflict = error.message.toLowerCase().includes('exists') || error.message.includes('duplicate');
